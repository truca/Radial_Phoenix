...
shot = love.audio.newSource("sound/shotcut.mp3")
shot:setVolume(1)
...
function countEnemies()
	i = 0
	for index, entity in pairs(ents.objects) do 
		i = i + 1
	end 
	return i == 0
end
...
function ents.updatePlayerSpeed(dt)
	if love.keyboard.isDown("d") then ents.activateSpeedBoost() end 
...
function ents.collisions(gamestate)
		...
		for j, enemy in pairs(ents.objects) do
			if ents.collision(enemy, shot, enemy.radius) then
				if (shieldblock) and (enemy.class == "shield") then 
					...
					if table.getn(ents.objects) then 
						win = true 
						stop = true
						gamestate = "win"
					end
...
function updateGame(dt, gamestate)
	...
	gamestate = ents.collisions(gamestate)
